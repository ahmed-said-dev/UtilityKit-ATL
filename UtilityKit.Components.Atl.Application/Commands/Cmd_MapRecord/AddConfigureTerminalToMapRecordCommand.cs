...
{
    public class AddConfigureTerminalToMapRecordCommand : IRequest
    {
        public AddConfigureTerminalToMapRecordRequestDto AddConfigureTerminalToMapRecordRequest { get; set; }
        public class Handler : IRequestHandler<AddConfigureTerminalToMapRecordCommand>
        {
            #region --- Variables
            ...
            #region --- Methods
            public async Task<Unit> Handle(AddConfigureTerminalToMapRecordCommand request, CancellationToken cancellationToken)
            {
                var mapRecordId = request.AddConfigureTerminalToMapRecordRequest.MapRecordId;
                ...
                if (mapRecordEntity.TerminalSettingsJson == null && request.AddConfigureTerminalToMapRecordRequest?.TerminalSettingsJson != null)
                {
                    _mapRecordRepository.UpdateMapRecordCompleteness(mapRecordEntity, ActionsEnum.AddTerminalSetting);
                }

                if (mapRecordEntity.TerminalSettingsJson != null && request.AddConfigureTerminalToMapRecordRequest?.TerminalSettingsJson == null)
                {
                    _mapRecordRepository.DeleteMapRecordCompleteness(mapRecordEntity, ActionsEnum.AddTerminalSetting);
                }

                mapRecordEntity.TerminalSettingsJson = _mapper.Map<TerminalSettings>(request.AddConfigureTerminalToMapRecordRequest.TerminalSettingsJson);
