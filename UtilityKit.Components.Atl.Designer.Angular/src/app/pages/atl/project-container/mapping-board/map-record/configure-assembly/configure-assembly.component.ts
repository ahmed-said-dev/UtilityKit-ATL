...
export class ConfigureAssemblyComponent implements OnInit {
  ...
  constructor(
    ...
    private _location: Location // private _assetGroupMappingService: AssetGroupMappingService
  ) {}
  ...
  get() {
    ...
    let assemblySettingsMapRecord =
      this._mapRecordService.getAssemblySettingsForEditQuery(this.mapRecordId);
  ...
  save() {
    ...
    assemblySettings.mapRecordId = this.mapRecordId!;
  ...
  delete() {
    this._swalService.confirmation(
      'Delete 3D',
      `Are you sure to delete this Assembly settings?`,
      () => {
        this._spinnerService.show();

        let assemblySettings: AddAssemblySettingsToMapRecordRequestDto =
          new AddAssemblySettingsToMapRecordRequestDto();
        assemblySettings.assemblySettingsJson = null;
        assemblySettings.mapRecordId = this.mapRecordId!;
        ...
        this._mapRecordService
          ...
          .subscribe({
            next: (result) => {
              this.close();
              this._toastrService.success(
                'Assembly settings has been deleted Successfully',
                'Success'
              );
              this.resetCurrentForm();
            },
