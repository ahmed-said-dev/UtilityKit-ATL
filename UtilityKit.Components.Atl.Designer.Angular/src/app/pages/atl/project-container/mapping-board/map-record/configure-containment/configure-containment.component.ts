...
export class ConfigureContainmentComponent implements OnInit {
  ...
  get() {
    ...
    let containmentSettingsInput =
      this._assetGroupMappingService.getContainmentSettings();
  ...
  save() {
    ...
    containmentSettings.mapRecordId = this.mapRecordId!;
  ...
  delete() {
    this._swalService.confirmation(
      'Delete 3D',
      `Are you sure to delete this Containment settings?`,
      () => {
        this._spinnerService.show();
        this.saving = true;
        let containmentSettings: AddContainmentSettingsToMapRecordRequestDto =
          new AddContainmentSettingsToMapRecordRequestDto();
        containmentSettings.containmentSettingsJson = null;
        containmentSettings.mapRecordId = this.mapRecordId!;
        ...
        this._mapRecordService
          ...
          .subscribe({
            next: (result) => {
              ...
              this.close();
              this._toastrService.success(
                'Containment settings has been deleted Successfully',
                'Success'
              );
              this.resetCurrentForm();
            },
