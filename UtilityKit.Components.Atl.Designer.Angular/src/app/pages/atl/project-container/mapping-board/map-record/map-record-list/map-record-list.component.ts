...
export class MapRecordListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._activatedRoute.params.subscribe((params) => {
      ...
      this.displayContainment = false;
      this.displayTerminal = false;
      this.displayAttachment = false;
      this.displayAssembly = false;
    ...
    this._mapRecordService
      ...
      .subscribe((result: GetMapRecordsByAssetGroupIdResponce) => {
        ...
        this.mapRecords.forEach((element) => {
          ...
          element.completenessCriteriaJson =
            element.completenessCriteriaJson?.filter((e) => e.comment);
        });
        // let indexs: number[] = [];
        // let orders: number[] = [];
        ...
        // console.log(orders);
        this._projectReportService.updateProjectCompletenessValueMsg(atlId);
        this._spinnerService.hide();
  ...
  openConfigureTerminal(mapRecord: GetMapRecordDto) {
    this._assetGroupMappingService.setDataSourceFields(mapRecord);
    let url = `atl/project-container/${this.atlId}/mapping-board/${this.undId}/map-record/terminals`;
    this._router.navigate([url], { queryParams: { id: mapRecord.id } });
  ...
  openConfigureContainment(mapRecord: GetMapRecordDto) {
    ...
    this._assetGroupMappingService.setContainmentSettings(
      containmentSettingsInput
    );
    let url = `atl/project-container/${this.atlId}/mapping-board/${this.undId}/map-record/containment`;
    this._router.navigate([url], { queryParams: { id: mapRecord.id } });
  }
  ...
  openConfigureStructre(mapRecord: GetMapRecordDto) {
    ...
    this._assetGroupMappingService.setStructureSettings(
      structuralAttachementInput
    );
    let url = `atl/project-container/${this.atlId}/mapping-board/${this.undId}/map-record/structure`;
    this._router.navigate([url], { queryParams: { id: mapRecord.id } });
  ...
  openConfigureAssembly(mapRecord: GetMapRecordDto) {
    // this._assetGroupMappingService.setAssemblySettings(
    //   structuralAttachementInput
    // );
    let url = `atl/project-container/${this.atlId}/mapping-board/${this.undId}/map-record/assembly`;
    this._router.navigate([url], { queryParams: { id: mapRecord.id } });
  }

  openConfigure3D(mapRecord: GetMapRecordDto) {
    this._assetGroupMappingService.setDataSourceFields(mapRecord);
    let url = `atl/project-container/${this.atlId}/mapping-board/${this.undId}/map-record/threeD`;
    this._router.navigate([url], { queryParams: { id: mapRecord.id } });
  }
  ...
  openFieldMapping(mapRecord: GetMapRecordDto) {
    this._assetGroupMappingService.setDataSourceFields(mapRecord);
    this._assetGroupMappingService.setDestinationFields(this.destinationFields);

    let url = `atl/project-container/${this.atlId}/mapping-board/${this.undId}/map-record/fieldMapping`;
    this._router.navigate([url], { queryParams: { id: mapRecord.id } });
  }
  ...
  setSubscription() {
    ...
    if (
      this.assetGroupMappingData.assetTableType ===
      AssetTableTypeEnum.DomainSubnetLine
    ) {
      this.display3D = true;
    }
    if (
